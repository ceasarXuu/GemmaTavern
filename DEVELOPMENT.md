...
1. `adb shell am start -W -n com.xuyutech.gemmatavern/.MainActivity`
2. `adb shell pidof com.xuyutech.gemmatavern`
3. `adb shell dumpsys activity activities | Select-String -Pattern 'com.xuyutech.gemmatavern/.MainActivity|ResumedActivity' -Context 0,1`

Do not assume the previous foreground state survived the install step.

On the current Android 16 test device, both `adb shell pidof com.xuyutech.gemmatavern`
and `adb shell pidof -s com.xuyutech.gemmatavern` have shown intermittent false-empty
results across different runs. Treat both as hints, not the source of truth.
Always fall back to:

1. `adb shell ps -A | Select-String -Pattern 'com.xuyutech.gemmatavern'`
2. `adb shell dumpsys activity activities | Select-String -Pattern 'com.xuyutech.gemmatavern/.MainActivity|ResumedActivity' -Context 0,1`
...
3. `adb shell am instrument -w -r -e class <fqcn> com.xuyutech.gemmatavern.test/androidx.test.runner.AndroidJUnitRunner`
...
## Gradle cache recovery note

If Gradle fails before project configuration with repeated
`Could not read workspace metadata from ...\.gradle\caches\8.10.2\...\metadata.bin`
errors, stop the daemon and move the broken cache subtree to a timestamped
backup directory instead of deleting it. Damaged `kotlin-dsl` and `transforms`
subtrees can be regenerated by running `./gradlew.bat --stop`, moving the
affected subtree under a sibling `*-backup` directory, and rerunning the build.
...
3. `adb shell am instrument -w -r -e class selfgemma.talk.domain.roleplay.usecase.WriteRoleplayDebugBundleAndroidTest com.xuyutech.gemmatavern.test/androidx.test.runner.AndroidJUnitRunner`
...
- `HUGGINGFACE_REDIRECT_URI=com.xuyutech.gemmatavern.auth:/oauth2redirect`
...
6. `adb shell am start -W -n com.xuyutech.gemmatavern/.MainActivity`,
7. verify foreground state with `adb shell pidof com.xuyutech.gemmatavern` and
   `adb shell dumpsys activity activities`.

On this device, `run-as com.xuyutech.gemmatavern sqlite3 ...` was not a reliable shortcut
...
3. `adb shell am start -W -n com.xuyutech.gemmatavern/.MainActivity` confirms launch.
