...
fun DownloadAndTryButton(
  ...
  var errorDialogTitle by remember { mutableStateOf("Unknown network error") }
  var errorDialogMessage by remember { mutableStateOf("Please check your internet connection.") }
        ...
        onTokenRequested = { tokenRequestResult ->
          when (tokenRequestResult.status) {
            ...
            TokenRequestResultType.FAILED -> {
              ...
              errorDialogTitle = "Token request failed"
              errorDialogMessage =
                tokenRequestResult.errorMessage ?: "Unable to complete the Hugging Face login flow."
  ...
  val startTokenExchange = fun() {
    val authRequest = modelManagerViewModel.getAuthorizationRequest()
    if (authRequest == null) {
      errorDialogTitle = "Hugging Face OAuth not configured"
      errorDialogMessage =
        "This build does not include Hugging Face OAuth settings. Add HUGGINGFACE_CLIENT_ID and HUGGINGFACE_REDIRECT_URI in Android/src/gradle.properties to enable gated model downloads."
      showErrorDialog = true
      checkingToken = false
      downloadStarted = false
      return
      ...
      if (needToDownloadFirst) {
        ...
        if (model.url.startsWith("https://huggingface.co")) {
          ...
          if (firstResponseCode == HttpURLConnection.HTTP_OK) {
            ...
            errorDialogTitle = "Unknown network error"
            errorDialogMessage = "Please check your internet connection."
            showErrorDialog = true
            return@launch
          }
  ...
  if (showAgreementAckSheet) {
    AgreementAckSheet(
      ...
      onLaunchAgreement = { intent -> agreementAckLauncher.launch(intent) },
    )
  }
