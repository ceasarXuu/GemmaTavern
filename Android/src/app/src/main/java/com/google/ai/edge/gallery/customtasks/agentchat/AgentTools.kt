...
class AgentTools() : ToolSet {
  ...
  /**
   * Emits an [AgentAction] without blocking the calling thread.
   *
   * The backing channel is [Channel.UNLIMITED] so [Channel.trySend] never returns failure under
   * normal operation. If the channel is unexpectedly closed (e.g. AgentTools instance shutdown),
   * we log and drop the action rather than crashing the LiteRT-LM tool callback thread.
   */
  private fun emit(action: AgentAction) {
    val result = _actionChannel.trySend(action)
    if (result.isFailure) {
      Log.w(TAG, "Failed to emit agent action ${action.name}; channel closed or full", result.exceptionOrNull())
    }
  }

  /**
   * Awaits a [Deferred] result from the calling LiteRT-LM tool thread.
   *
   * `runBlocking` is needed because LiteRT invokes [@Tool] methods synchronously off a worker
   * thread that is not driving a coroutine. We wrap so cancellation of the deferred is propagated
   * out and the deferred is cleaned up if the thread is interrupted.
   */
  private fun <T> awaitBlocking(deferred: Deferred<T>): T =
  ...
  fun runJs(
    ...
    if (skill == null) {
      ...
      return mapOf("error" to "Skill \"${scriptName}\" not found", "status" to "failed")
    }
    ...
    if (skill.requireSecret) {
      ...
      if (savedSecret == null || savedSecret.isEmpty()) {
        ...
        emit(askAction)
        secret = awaitBlocking(askAction.result)
        if (secret.isNotEmpty()) {
          skillManagerViewModel.dataStoreRepository.saveSecret(
            key = getSkillSecretKey(skillName = skillName),
            value = secret,
          )
          // Intentionally do not log secret contents (not even a prefix) to avoid PII leakage.
          Log.d(TAG, "Got secret from ask info dialog (length=${secret.length}).")
        } else {
    ...
    val url =
      skillManagerViewModel.getJsSkillUrl(skillName = skillName, scriptName = scriptName)
        ?: return mapOf("result" to "JS Skill URL not set properly or skill not found")
    ...
    emit(callAction)
    val result = awaitBlocking(callAction.result)
  ...
  fun runIntent(
    ...
    return if (IntentHandler.handleAction(context, intent, parameters)) {
      mapOf("action" to intent, "parameters" to parameters, "result" to "succeeded")
    } else {
      mapOf("action" to intent, "parameters" to parameters, "result" to "failed")
    }
  }
